...
import {
  ...
  PropertyHeroCard,
  ScoreProgressCard,
} from "@/components/real-estate/report";
import {
  BrainIcon,
  GaugeIcon,
  LocationIcon,
          ...
          <>
            <section className="mt-8">
              <PropertyHeroCard
                ...
                recommendation={resultAction}
                score={overallScore}
                listingPrice={result.listing_price}
                fairValue={result.fair_value}
                monthlyRent={
                  result.estimated_monthly_rent
                }
                cashFlow={
          ...
          <div className="mt-4 grid gap-3">
            ...
            <div className="rounded-[44px] border border-white/10 bg-white/[0.06] p-6 shadow-[0_40px_140px_rgba(0,0,0,0.45)] backdrop-blur-2xl md:p-8">
              <div className="flex flex-col gap-5 md:flex-row md:items-start md:justify-between">
                ...
                <div
                  className={`flex shrink-0 items-center gap-3 rounded-[18px] border px-4 py-3 ${getVerdictStyle(
                    result.status,
                  )}`}
                >
                  <div>
              ...
              <div className={result ? "mt-4" : "mt-6"}>
                <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                  <div className="min-w-0">
                    ...
                    <h3 className="mt-2 break-words text-2xl font-black tracking-[-0.045em] [overflow-wrap:anywhere]">
                      How this opportunity scores.
                    </h3>
                  </div>

                  <p className="max-w-xl text-sm leading-6 text-white/35">
                    A visual breakdown of deal quality, market outlook,
                    neighborhood strength, and AI confidence.
                  </p>
                </div>

                <div className="mt-5 grid auto-rows-fr gap-4 sm:grid-cols-2 2xl:grid-cols-4">
                  <ScoreProgressCard
                    title="Deal Score"
                    value={overallScore}
                    tone="emerald"
                    ...
                    description="Overall investment opportunity"
                  />

                  <ScoreProgressCard
                    title="Forecast"
                    value={result.forecast_score ?? 0}
                    tone="cyan"
                    icon={
                      <TrendUpIcon className="h-5 w-5" />
                    }
                    description={
                      result.forecast_outlook ||
                  ...
                  <ScoreProgressCard
                    ...
                    description={
                      result.neighborhood_grade
                        ? `Neighborhood grade ${result.neighborhood_grade}`
        ...
        >
          {guestSubmitting ? "Unlocking..." : "Unlock Free"}
            ...
            <div className="grid min-w-0 gap-6 xl:grid-cols-3">
              <div className="flex min-w-0 flex-col rounded-[32px] border border-cyan-300/15 bg-cyan-300/[0.055] p-6 backdrop-blur-2xl">
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0">
                    ...
                    <h3 className="mt-3 break-words text-2xl font-black tracking-[-0.045em] [overflow-wrap:anywhere]">
                      {result.forecast_outlook ||
                        "Forecast Ready"}
                    </h3>
                  </div>

                  <div className="shrink-0 rounded-[16px] border border-cyan-300/15 bg-cyan-300/10 px-4 py-3 text-right">
                    ...
                    <p className="mt-1 text-xl font-black text-cyan-100">
                      {Number(
                        result.expected_appreciation ?? 0,
                      ).toFixed(1)}
                      %
                    </p>
                  </div>
                ...
                <div className="mt-6 flex-1 space-y-3">
                  {(result.forecast_reasons ?? []).length >
                  0 ? (
                    (result.forecast_reasons ?? []).map(
                      (reason) => (
                        <div
                          key={reason}
                          className="flex min-w-0 gap-3 rounded-[18px] border border-white/10 bg-black/20 p-4 text-sm leading-6 text-white/58"
                        >
                    ...
                    <p className="rounded-[18px] border border-dashed border-white/10 bg-black/15 p-4 text-sm leading-6 text-white/35">
                      Forecast reasoning will appear when
              ...
              <div className="flex min-w-0 flex-col rounded-[32px] border border-amber-300/15 bg-amber-300/[0.05] p-6 backdrop-blur-2xl">
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0">
                  ...
                  <span className="shrink-0 rounded-full border border-amber-300/15 bg-amber-300/10 px-3 py-2 text-xs font-bold text-amber-100">
                    Area Intelligence
                  </span>
                </div>

                <div className="mt-6 flex-1 space-y-3">
                  {(result.neighborhood_reasons ?? [])
                    .length > 0 ? (
                    (
                      result.neighborhood_reasons ?? []
                    ).map((reason) => (
                    ...
                    <p className="rounded-[18px] border border-dashed border-white/10 bg-black/15 p-4 text-sm leading-6 text-white/35">
                      Neighborhood details will appear when
              ...
              <div className="flex min-w-0 flex-col rounded-[32px] border border-violet-300/15 bg-violet-300/[0.05] p-6 backdrop-blur-2xl">
                <div>
                ...
                <div className="mt-6 grid flex-1 gap-3">
                  {[
                    ...
                    <div
                      key={label}
                      className="flex min-w-0 items-center justify-between gap-4 rounded-[17px] border border-white/10 bg-black/20 px-4 py-3"
                    >
                      <span className="min-w-0 truncate text-sm text-white/38">
                        {label}
                      </span>

                      <span className="shrink-0 text-sm font-bold text-white/75">
                        {value}
                      </span>
                    </div>
                  ))}
