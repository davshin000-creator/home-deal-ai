...
import type {
  SearchDealCandidate,
  SearchSuggestion,
} from "@/lib/real-estate/search/suggestion-types";
import {
  evaluateSearchSuggestion,
  selectBestComparableAlternative,
  shouldWaitOnSearch,
} from "@/lib/real-estate/search/suggestion-engine";

type Deal = SearchDealCandidate;
...
const ALTERNATIVE_MARKETS: Record<
  string,
  {
    city: string;
    state: string;
...
export default function DealsPage() {
  ...
  async function findDeals(customCity?: string, customState?: string, customMaxPrice?: string) {
    ...
    try {
      ...
      const data =
        (await response.json()) as FindDealsResult;
      ...
      const originalDeals =
        [...(data.deals || [])].sort(
          (a, b) =>
            getDealOverallScore(b) -
            getDealOverallScore(a),
        );

      const originalCandidate =
        originalDeals[0] ?? null;

      const marketKey =
        `${finalCity.trim().toLowerCase()},${finalState
          .trim()
          .toLowerCase()}`;

      const alternativeMarket =
        ALTERNATIVE_MARKETS[marketKey];
      ...
      if (
        ...
        try {
          const alternativeResponse =
            await fetch("/api/find-deals", {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
              },
              body: JSON.stringify({
                city: alternativeMarket.city,
                state: alternativeMarket.state,
                max_price:
          ...
          if (alternativeResponse.ok) {
            const alternativeData =
              (await alternativeResponse.json()) as FindDealsResult;

            const alternativeDeals =
              [...(alternativeData.deals || [])].sort(
                (a, b) =>
                  getDealOverallScore(b) -
                  getDealOverallScore(a),
              );
            ...
            const evaluatedSuggestion =
              evaluateSearchSuggestion({
                type: "nearby_market",
                title:
                  `Consider ${alternativeMarket.city}`,
                summary:
                  "A nearby market may offer a stronger fit.",
                originalSearch: {
                  city: finalCity,
                  state: finalState,
                  maxPrice:
                ...
                alternativeSearch: {
                  city:
                    alternativeMarket.city,
                  state:
                    alternativeMarket.state,
                  maxPrice:
                    Number(finalMaxPrice),
                },
...
if (
              evaluatedSuggestion.recommended
            ) {
              setSuggestion(
                evaluatedSuggestion,
              );
  ...
  const shouldWait =
    !suggestion &&
    shouldWaitOnSearch(topDeal ?? null);
            ...
            {suggestion ? (
              <>
                ...
                <p className="mt-3 text-sm leading-6 text-white/45">
                  {suggestion.label === "better_match"
                    ? "A stronger overall option was found."
                    : "Similar quality with stronger economics."}
                </p>
                ...
                {suggestion.alternative.candidate ? (
                  <div className="mt-7 border-t border-white/10 pt-6">
                    ...
                    <p className="mt-2 text-base font-semibold text-white">
                      {suggestion.alternative.candidate.address}
                    </p>

                    <a
                      href={`/analyze?address=${encodeURIComponent(
                        suggestion.alternative.candidate.address,
                      )}&listing_price=${encodeURIComponent(
                        String(
                          suggestion.alternative.candidate.listing_price,
                        ),
                      )}`}
                      className="mt-5 inline-flex rounded-full bg-white px-5 py-3 text-sm font-semibold text-black transition hover:bg-neutral-200"
                    >
                      Analyze
                    </a>
                  </div>
              ...
              <>
                ...
                <p className="mt-3 text-sm leading-7 text-white/45">
                  Nothing is compelling enough to justify a move yet.
                </p>
                ...
                {topDeal ? (
                  <div className="mt-7 rounded-[26px] border border-white/10 bg-black/25 p-5">
                    ...
                    <p className="mt-2 text-sm text-white/35">
                      Keep watching for a stronger price or economics.
                    </p>
                  </div>
                ) : null}
              </>
              ...
              <>
                ...
                <p className="mt-2 text-sm leading-6 text-white/45">
                  This is currently the strongest option in your search.
                </p>
              ...
              <>
                <h2 className="mt-4 text-3xl font-semibold tracking-[-0.05em]">
                  Look beyond your filters.
                </h2>

                <p className="mt-4 text-sm leading-7 text-white/45">
                  Search a market and Nestrova will surface stronger matches and useful alternatives when they exist.
                </p>
              </>
            )}
          </aside>
            ...
            <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
              <div>
                ...
                <h2 className="mt-2 text-4xl font-semibold tracking-[-0.05em] md:text-5xl">
                  Top results in {result.city}, {result.state}
                </h2>
