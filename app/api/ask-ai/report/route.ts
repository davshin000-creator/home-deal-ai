...
function buildReportHtml({
  ...
  const annualAppreciation =
    safeNumber(property.expected_appreciation) / 100;

  const forecastRows = [0, 1, 3, 5].map((year) => ({
    year,
    value: Math.round(
      fairValue * Math.pow(1 + annualAppreciation, year)
    ),
  }));

  const maxForecast = Math.max(
    ...forecastRows.map((item) => item.value),
    1
  );

  const safeAddress = escapeHtml(property.address);
  const safeStatus = escapeHtml(property.status || "Analyzed");
  const safeSummary = escapeHtml(
    property.summary ||
      "Nestrova analyzed this property using valuation, rent, cash-flow, and market factors."
  );

  const safeInvestorType = escapeHtml(investorType);
  ...
  const locked = !isPro
    ? `<div class="locked">
        ...
        <p>
          This preview includes the executive summary and key metrics.
          Pro includes negotiation strategy, risk analysis, exit strategy,
          and the full downloadable report.
        </p>
      </div>`
    : "";
  ...
  const proOnlySections = isPro
    ? `
      <section class="page">
        ...
        <div class="ai-text">${safeAiText}</div>
      </section>
      ...
      <section class="page">
        ...
        <p>
          ...
          <strong>${money(forecastRows[3].value)}</strong>
        </p>

        <p>
          Best suited for:
          <strong>${safeInvestorType}</strong>
        </p>
      </section>
      ...
      <section class="page">
        ...
        <p>
          This report is informational only and is not financial,
...
<body>
  ...
  <section class="page">
    ...
    <p class="muted">
      Simple projection based on current fair value
  ...
  try {
    ...
    if (!property?.address) {
      return NextResponse.json(
        {
          error:
            "Property analysis result is required.",
        },
        { status: 400 }
      );
    }
    ...
    const propertyContext = {
      ...
      gross_rent_yield:
        safeNumber(property.gross_rent_yield),
    ...
    let aiText =
      "Preview mode. Upgrade to Pro to unlock the full negotiation, risk, and exit strategy sections.";
          ...
          body: JSON.stringify({
            ...
            messages: [
...

              {
                ...
                content:
                  `Create a professional AI report section for this property. Include negotiation strategy, risk analysis, exit strategy, and final recommendation. Property data: ${JSON.stringify(
                    propertyContext,
                    null,
                    2
                  )}`,
    ...
    return NextResponse.json(
      {
        error:
          "Unexpected report server error.",
      },
      { status: 500 }
    );
  }
