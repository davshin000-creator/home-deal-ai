...
export function generateOfferStrategies(input: OfferInput) {
  const days = input.daysOnMarket ?? 21;
  const discount =
    input.fairValue > 0 ? (input.fairValue - input.askingPrice) / input.fairValue : 0;
  ...
  return [
    {
      ...
      offer: roundToNearest(base * (1 - Math.min(0.07, leverage + 0.025))),
