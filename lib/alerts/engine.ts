...
export async function runAlertEngine(): Promise<AlertEngineResult> {
  ...
  const [tradingState, watchlistResponse] = await Promise.all([
    fetchTradingState(),
    supabase
  ...
  if (watchlistResponse.error) {
    throw new Error(watchlistResponse.error.message);
  }

  const watchlists =
    (watchlistResponse.data ?? []) as WatchlistAlertInput[];
  ...
  for (const item of watchlists) {
    ...
    candidates.push(
      ...evaluateWatchlistItem(item, opportunity),
    );
  }
  ...
  for (const candidate of candidates) {
    const { error } = await supabase
      ...
      .insert({
        ...
        watchlist_id: candidate.watchlistId,
        ...
        triggered_value: candidate.triggeredValue,
        threshold_value: candidate.thresholdValue,
        source_updated_at:
          tradingState.generated_at ?? null,
  ...
  return {
    evaluatedWatchlists: watchlists.length,
    matchedAssets,
    candidateAlerts: candidates.length,
    insertedAlerts,
    skippedDuplicates,
  };
}
